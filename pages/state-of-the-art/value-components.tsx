...
function Yes() {
  return (
    <span style={{ color: COLOR_GREEN, fontSize: "1.9em", display: "block" }}>
      &#x2713;
    </span>
  );
}
function No() {
  return (
    <span style={{ color: COLOR_RED, fontSize: "1.6em", display: "block" }}>
      &#x2717;
    </span>
  );
}
...
function TableRef({ children }: { children: JSX.Element[] }) {
  ...
  return (
    <p
      style={
        {
          /* marginBottom: 10 /*, fontSize: "0.87em"*/
        }
      }
    >
      <em>{title}</em>: {body}
...
function ValueTable({
  ...
  function TH({ children, ...rest }: { children: string; rest?: never }) {
    ...
    const child = skip_links ? (
      value_name
    ) : (
      <a href={get_value_link(value_name)}>{value_name}</a>
    );
